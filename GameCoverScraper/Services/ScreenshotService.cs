...
public static class ScreenshotService
{
    ...
    public static string? CaptureActiveWindow()
    {
        try
        {
            ...
            if (hWnd == IntPtr.Zero)
            {
                AppLogger.Log("Screenshot: No foreground window found.");
                return null;
            }

            if (!TryGetWindowBounds(hWnd, out var rect))
            {
                AppLogger.Log("Screenshot: Failed to get window bounds.");
                return null;
            }
            ...
            if (width <= 0 || height <= 0)
            {
                AppLogger.Log("Screenshot: Window has invalid dimensions.");
                return null;
            }
            ...
            AppLogger.Log($"Screenshot: Saved to '{filePath}'.");
            return filePath;
        }
        catch (Exception ex)
        {
            AppLogger.Log($"Screenshot: Error capturing window - {ex.Message}");
            _ = BugReport.LogErrorAsync(ex, "Error capturing screenshot.");
            return null;
        }
    }
